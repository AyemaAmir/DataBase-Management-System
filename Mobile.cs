...
namespace BOOKSWOOKS_PROJECT
{
    ...
    public partial class Mobile : Form
    {
        ...
        {
            InitializeComponent();
            this.Load += new EventHandler(Mobile_Load); // Subscribe to the Load event
            FormClosing += Mobile_LoadClosing;

        }
        private void Mobile_LoadClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
        ...
        {
            string query = "SELECT Product_ID, Product_Name, Brand, Description, Price, Cost,  Qty, Items_per_unit,Profit FROM Product WHERE Category_ID = 4";
