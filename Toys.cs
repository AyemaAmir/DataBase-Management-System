...
namespace BOOKSWOOKS_PROJECT
{
    public partial class Toys : Form
    {
        ...
        {
            InitializeComponent();
            this.Load += new EventHandler(Toys_Load); // Subscribe to the Load event
            FormClosing += Toys_LoadClosing;

        }
        private void Toys_LoadClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
        ...
        {
            string query = "SELECT Product_ID, Product_Name, Brand, Description, Price, Cost, Qty, Items_per_unit,Profit FROM Product WHERE Category_ID = 3";
