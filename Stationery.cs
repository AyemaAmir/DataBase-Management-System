...
namespace BOOKSWOOKS_PROJECT
{
    ...
    public partial class Stationery : Form
    {
        ...
        {
            InitializeComponent();
            this.Load += new EventHandler(Stationery_Load); // Subscribe to the Load event
            FormClosing += Stationery_LoadClosing;

        }
        private void Stationery_LoadClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
        ...
        {
            string query = "SELECT Product_ID, Product_Name, Brand, Description, Price, Cost,  Qty, Items_per_unit,Profit FROM Product WHERE Category_ID = 1";
